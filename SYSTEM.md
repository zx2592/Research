...
4. 搜索限额：**逐 workflow 配置，由代码强制**——预算声明在 `core/settings.py` 的
   `WorkflowBudgetSettings`（如 `/scan` 28 点、`/deep` 14 点、`/quick` 2 点、`/add` 0 点），
   由 `ToolFactory` 在工具层拦截，并由 `WorkflowRunner` 注入提示词。**不要在别处另写一个数字**——
   此前这里写死的「最多 8 次」与各 workflow 正文声明的次数互相矛盾，现已统一到单一真理源
